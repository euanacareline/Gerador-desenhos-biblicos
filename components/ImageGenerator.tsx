...
import { generateImagePrompt, generateImage, generateSpeech, getVerseText, generateVoicePreview } from '../services/geminiService';
...
const ImageGenerator: React.FC = () => {
  ...
  const [voiceType, setVoiceType] = useState('masculina'); // 'masculina' | 'feminina'
  ...
  const [previewLoading, setPreviewLoading] = useState<'masculina' | 'feminina' | null>(null);
  ...
  const handleGenerateAudioClick = useCallback(async () => {
    ...
    try {
        const audioBase64 = await generateSpeech(textForNarration, voiceType === 'masculina' ? 'adulta' : 'infantil');
  ...
  const handlePreviewVoice = useCallback(async (vType: 'masculina' | 'feminina') => {
    if (previewLoading) return;
    setPreviewLoading(vType);
    setPreviewError(null);
    try {
        const audioBase64 = await generateVoicePreview(vType === 'masculina' ? 'adulta' : 'infantil');
        const audioBlob = createWavBlob(audioBase64);
        const url = URL.createObjectURL(audioBlob);
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col justify-end">
                    ...
                    <div className="flex items-center justify-around gap-4 bg-slate-900 border border-slate-600 rounded-lg p-2 h-full">
                        {(['masculina', 'feminina'] as const).map((type) => (
                            <div key={type} className="flex items-center">
                                <label className="flex items-center gap-2 cursor-pointer text-base">
                                    <input
                                        ...
                                        onChange={(e) => setVoiceType(e.target.value as 'masculina' | 'feminina')}
                                ...
                                <button
                                    onClick={() => handlePreviewVoice(type)}
                                    disabled={isAnyLoading || !!previewLoading}
                                    className="ml-2 p-1 rounded-full hover:bg-slate-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                    aria-label={`Ouvir prévia da voz ${type}`}
                                >
                                    {previewLoading === type 
