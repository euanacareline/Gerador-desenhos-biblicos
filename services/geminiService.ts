...
/**
 * Generates a short audio presentation of the application.
 * @returns A promise that resolves to the base64 encoded audio string.
 */
export const generatePresentationAudio = async (): Promise<string> => {
    try {
        ...
        const audioResponse = await ai.models.generateContent({
          ...
          config: {
            ...
            speechConfig: {
                voiceConfig: {
                  prebuiltVoiceConfig: { voiceName: 'Puck' }, // A professional adult voice
                },
            },
          },
        });
...
/**
 * Generates a short audio preview for a given voice type.
 * @param voiceType - The type of voice ('adulta' or 'infantil').
 * @returns A promise that resolves to the base64 encoded audio string.
 */
export const generateVoicePreview = async (voiceType: 'adulta' | 'infantil'): Promise<string> => {
    try {
        const voiceName = voiceType === 'adulta' ? 'Puck' : 'Kore';
